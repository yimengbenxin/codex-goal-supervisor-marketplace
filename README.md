# Codex Goal Supervisor - Full Marketplace

This repository is the official `full` Codex marketplace channel for [Codex Goal Supervisor](https://github.com/yimengbenxin/codex-goal-supervisor), an execution-convergence tool for long-running Codex work.
...
- Automatic updates: included and pinned to the `full` channel.
- Feedback: Remote feedback support is included but disabled by default and requires explicit project consent.
...
```bash
codex plugin marketplace add yimengbenxin/codex-goal-supervisor-marketplace --ref main
codex plugin add codex-goal-supervisor@goal-supervisor
```
